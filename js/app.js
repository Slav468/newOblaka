(() => {
    ...
    (() => {
        ...
        class Popup {
            ...
            open(selectorValue) {
                if (bodyLockStatus) {
                    ...
                    } else this.popupLogging(`Упс, такого попапа нет. Проверьте передаваемые данные. `);
                }
            }
            close(selectorValue) {
        ...
        let gotoblock_gotoBlock = (targetBlock, noHeader = false, speed = 500, offsetTop = 0) => {
            ...
            } else functions_FLS(`[gotoBlock]: Упс, такого блока не существует: ${targetBlock}`);
        };
        function formFieldsInit(options = {
            ...
            document.body.addEventListener("focusin", (function(e) {
                ...
                if ((targetElement.tagName === "INPUT" || targetElement.tagName === "TEXTAREA") && targetElement.hasAttribute("data-validate")) {
        ...
        let formValidate = {
            ...
            toggleBtnAttr(error, form) {
                ...
                if (!error) btn.removeAttribute("disabled"); else btn.setAttribute("disabled", "");
            },
            checkValidate(form) {
                let error = 0;
                let validateInput = form.querySelectorAll("*[data-required]");
                if (validateInput.length) validateInput.forEach((validateItem => {
                    const type = validateItem.getAttribute("type");
                    if (type === "checkbox") return validateItem.checked ? error : error++;
                    if (type !== "checkbox") return validateItem.value.trim() ? error : error++;
                }));
                return error;
            },
            validateInput(formRequiredItem) {
                ...
                if (formRequiredItem.dataset.required === "email") {
                    ...
                    formRequiredItem.classList.add("validated");
                    this.removeError(formRequiredItem);
                } else if (formRequiredItem.dataset.required === "number") if (this.numberTest(formRequiredItem)) {
                    this.addError(formRequiredItem);
                    error++;
                } else {
                    formRequiredItem.classList.add("validated");
            ...
            phoneTest(formRequiredItem) {
                return !/^\+375[\s|-]?[0-9]{2}[\s|-]?[0-9]{3}[\s|-]?[0-9]{2}[\s|-]?[0-9]{2}$/.test(formRequiredItem.value);
            },
        ...
        function initSliders() {
            ...
            if (document.querySelector(".request-slider")) new swiper_core_Swiper(".request-slider", {
                modules: [ Navigation ],
                slidesPerView: "auto",
                spaceBetween: 20,
                speed: 300,
...
***************************************************************************** */
        ...
        if (document.querySelector(".dropdown[data-click]")) {
            const triggers = document.querySelectorAll(".dropdown[data-click] .dropdown-head");
        ...
        function script_removeDrop(array, selector, parentNode) {
            array.forEach((item => {
                item.addEventListener("click", (e => {
                    ...
                    let isParent = false;
                    for (let child of childFirstCircle) child === dropEl ? isParent = true : false;
                    if (isParent) {
        ...
        const contactsPhones = document.querySelectorAll(".contacts .contacts-phone .mobile-back");
        ...
        function setBtnState() {
            ...
            forms.forEach((form => {
                ...
                if (formCheckBox) formCheckBox.addEventListener("change", (e => {
        ...
        if (document.querySelector(".count")) {
            ...
            for (let count of countsBlocks) {
                ...
                countInputs.addEventListener("input", (e => {
                    e.preventDefault();
                    if (+countInputs.value === 0) countInputs.value = 1; else countInputs.value = countInputs.value.replace(/\D/gm, "");
                }));
            }
        }
        ...
        function hideMenuItem() {
            ...
            for (let menu of menus) {
                ...
                const menuLastItemList = menuLastItem ? menuLastItem.querySelector(".menu__list") : null;
                const menuLastItemListChildren = menuLastItemList !== null ? menuLastItemList.children : null;
                ...
                if (menuLastItem === null && totalWidthItem > width) {
                    addMenuEl(menuList);
                    hideMenuItem();
                    const dropButton = document.querySelectorAll("[data-menu-group] > .menu__svg");
                    dropMenu(dropButton, ".menu__item", "touchstart");
                ...
                if (totalWidthItem > width && menuListChildren.length - 1 > 0 && menuLastItem) {
                    menuRemoveChild(menuList, menuListChildren, menuLastItemList);
                    hideMenuItem();
                }
                if (totalWidthItem + 110 < width && menuListChildren.length < 8 && menuLastItem) menuBackItem(menuList, menuLastItemList, menuLastItemListChildren);
            }
            if (document.querySelector("[data-menu-group]") && document.querySelector("[data-menu-group] .menu__list").children.length <= 0) removeMenuEl("[data-menu-group]");
        }
        ...
        function eventAccord() {
            ...
            accordingAll.forEach((accordion => {
                ...
                if (accordion.dataset.toggle === "true") for (const item of accordionChildren) item.addEventListener("click", (e => {
                    item.classList.toggle("drop");
                })); else for (const item of accordionChildren) item.addEventListener("click", (e => {
                    const target = e.target;
                    if (target.closest(".accordion__head")) if (item.classList.contains("drop")) item.classList.toggle("drop"); else {
        ...
        if (document.querySelector(".hidden")) document.addEventListener("scroll", (() => {
        ...
        if (header.hasAttribute("data-fixed")) window.addEventListener("scroll", (e => {
            fixedHeader();
        }));
        ...
        if (document.querySelector(".card-controls")) {
            ...
            for (let card of cardControls) card.addEventListener("click", (e => {
                const target = e.target;
                if (target.closest(".card-controls__wrapper")) toggleActive(target.closest(".card-controls__wrapper"));
            }));
        }
        if (document.querySelector(".bar-right")) {
            ...
            for (let card of cardControls) card.addEventListener("click", (e => {
                const target = e.target;
                if (target.closest(".bar-right__item")) toggleActive(target.closest(".bar-right__item"));
            }));
        }
        ...
        function setOptions() {
            ...
            lists.forEach((list => {
                list.addEventListener("click", (e => {
                    ...
                    if (target.matches(".options__item")) target.classList.toggle("active");
                }));
            }));
        }
        ...
        function setVariable() {
            ...
            for (let parent of parentVariablesItems) {
                ...
                for (let child of children) {
                    ...
                    childTitleValue ? childTitle.innerHTML = `${childTitleValue}: <span>Не выбрано</span>` : false;
                    for (let button of buttons) {
                        setActiveVariable(buttons, button);
                        if (button.hasAttribute("data-color-value")) button.style.backgroundColor = `${button.dataset.colorValue}`;
                        if (button.hasAttribute("data-value")) button.textContent = `${button.dataset.value}`;
        ...
        function setTitleTextActiveEl(button) {
            ...
            for (let child of parent.children) if (child.classList.contains("active")) span.textContent = `${child.dataset.title}`;
        }
        ...
        function setActive(parent, parentBlock) {
            ...
            for (let child of childrenItem) child.addEventListener("click", (e => {
                e.preventDefault();
        ...
        if (document.querySelector(".basket-list")) {
            ...
            } else script_removeElement(".basket-list__wrapper", ".basket-item__remove", ".basket-item", ".basket-reset", addEl);
        }
        function script_removeElement(el, trigger, parent, check, addEl) {
            ...
            for (let element of elements) element.addEventListener("click", (e => {
                const target = e.target;
        ...
        if (document.querySelector(".formalization-form")) {
            ...
            for (let i = 0; i < listItem.length; i++) {
                ...
                const changeButton = listItem[i].querySelector(".formalization-form__change");
                ...
                nextButton?.addEventListener("click", (e => {
                    ...
                    if (checked) toggleActiveFormEl("next", listItem, i); else return;
                }));
        ...
        function toggleActiveFormEl(move, list, index) {
            ...
            if (move === "next") {
                ...
                if (list[index + 1].matches(".resolved")) {
                    for (let i = 0; i <= list.length - 1; i++) if (!list[i].matches(".resolved") && list[i - 1].matches(".resolved")) list[i].classList.add("active");
                } else {
                    list[index + 1].classList.add("active");
                    list[index + 1].classList.remove("resolved");
                }
        ...
        if (document.querySelector("[data-search-activate]") && document.querySelector(".search")) {
            const buttonActivateSearch = document.querySelectorAll("[data-search-activate]");
            const closeSearchBtn = document.querySelector(".search-form__close");
            for (let button of buttonActivateSearch) button.addEventListener("click", (e => {
                e.preventDefault();
        ...
        function toggleActiveSearchForm() {
            ...
            if (search.matches(".active")) removeActive(".search"); else {
        ...
        if (document.querySelector(".search")) {
            ...
            const mutationObserver = new MutationObserver((function(mutations) {
                for (let mutation of mutations) if (mutation.type === "childList") initSearchSlider();
            }));
